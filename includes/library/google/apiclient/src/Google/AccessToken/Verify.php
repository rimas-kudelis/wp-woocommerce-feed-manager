 ...
 */
use RexFeed\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use RexFeed\Firebase\JWT\SignatureInvalidException;
...
use RexFeed\Stash\Driver\FileSystem;
use RexFeed\Stash\Pool;
/**
...
class Google_AccessToken_Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('RexFeed\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'RexFeed\\Firebase\\JWT\\JWT';
        }
    ...
    {
        if (\class_exists('RexFeed\\phpseclib\\Crypt\\RSA')) {
            return 'RexFeed\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('RexFeed\\phpseclib\\Math\\BigInteger')) {
            return 'RexFeed\\phpseclib\\Math\\BigInteger';
        }
    ...
    private function getOpenSslConstant()
    {
        ...
        if (\class_exists('RexFeed\\Crypt_RSA')) {
 ...
 *
 */
\class_alias('RexFeed\\Google_AccessToken_Verify', 'Google_AccessToken_Verify', \false);
