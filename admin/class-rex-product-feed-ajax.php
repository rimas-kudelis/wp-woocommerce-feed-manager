...
class Rex_Product_Feed_Ajax {
	...
	public static function category_mapping( $payload ) {
		...
		if ( '' !== $wpfm_hash && array_key_exists( $wpfm_hash, $category_map ) ) {
			wp_send_json_success(
				array(
					'status'   => $status,
					'location' => esc_url( admin_url( 'admin.php?page=category_mapping' ) ),
				)
			);
		}
		if ( '' !== $wpfm_hash ) {
	...
	public static function clear_batch() {
		...
		$wpdb->update(
			$wpdb->postmeta,
			array( 'meta_value' => 'completed' ), //phpcs:ignore
			array( 'meta_key' => '_rex_feed_status' ), //phpcs:ignore
		);
	 ...
	 * @return array
	 */
	public static function wpfm_bf_notice_dismiss() {
		$current_time = time();
	...
	public static function rex_feed_custom_filter_option( $payload ) {
		$feed_id       = !empty( $payload[ 'feed_id' ] ) ? $payload[ 'feed_id' ] : '';
		$filter_option = !empty( $payload[ 'button_text' ] ) ? $payload[ 'button_text' ] : 'removed';
		$event         = !empty( $payload[ 'button_event' ] ) ? $payload[ 'button_event' ] : 'on_load';
