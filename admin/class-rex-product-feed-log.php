...
class Rex_Product_Feed_Log {
	...
	function __construct() {
		...
		add_action( 'rex_logging_prune_routine', array( $this, 'prune_logs' ) );
	...
	public function prune_logs(){
		...
		$should_we_prune = apply_filters( 'rex_logging_should_we_prune', false );
	 ...
	 *
	 * @filter rex_logging_force_delete_log         Allows user to override the force delete setting which bypasses the trash
	 */
	private function prune_old_logs( $logs ){

		$force = apply_filters( 'rex_logging_force_delete_log', true );
	 ...
	 *
	 * @filter rex_logging_prune_when           Users can change how long ago we are looking for logs to prune
	 * @filter rex_logging_prune_query_args     Gives users access to change any query args for pruning
	 */
	private function get_logs_to_prune(){

		$how_old = apply_filters( 'rex_logging_prune_when', '2 weeks ago' );
		...
		$old_logs = get_posts( apply_filters( 'rex_logging_prune_query_args', $args ) );
	...
	*/
	...
	public function register_post_type() {
		...
		register_post_type( 'wp_log', apply_filters( 'rex_logging_post_type_args', $log_args ) );
...
$GLOBALS['wp_logs'] = new Rex_Product_Feed_Log();
