...
use RexTheme\RexHeurekaAvailability\Item;
...
class Feed
{
     ...
     * Feed constructor
     */
    public function __construct($wrapper = false, $itemName = 'item', $namespace = null, $version = '', $rss = 'rss', $stand_alone = false, $wrapperel = '', $namespace_prefix = '')
    {
    ...
    private function addItemsToFeed()
    {
        ...
        foreach ($this->items as $item) {
            ...
            if ( !empty($this->channelName) ) {
                $feedItemNode = $this->feed->{$this->channelName}->addChild($this->itemName);
            }else{
                $feedItemNode = $this->feed->addChild($this->itemName);
            }

            foreach ($item->nodes() as $itemNode) {
                if ( 'id' === $itemNode->get( 'name' ) ) {
                    $feedItemNode->addAttribute( 'id', $itemNode->get( 'value' ) );
                }
                elseif ( 'orderDeadline' === $itemNode->get( 'name' ) ) {
                    $feedItemNode->addChild( 'delivery_time', self::calculate_delivery_pickup_time( $itemNode->get( 'value' ) ) )->addAttribute( $itemNode->get( 'name' ), $itemNode->get( 'value' ) );
                }
                elseif ( in_array( $itemNode->get( 'name' ), $depots ) && is_array( $itemNode->get( 'value' ) ) ) {
                    $feed_depot = $feedItemNode->addChild( 'depot' );
                    foreach( $itemNode->get( 'value' ) as $key => $value ) {
                        if( 'id' === $key ) {
                            $feed_depot->addAttribute( $key, $value );
                        }
                        elseif( 'orderDeadline' === $key ) {
                            $feed_depot->addChild( 'pickup_time', self::calculate_delivery_pickup_time( $value ) )->addAttribute( $key, $value );
                        }
                        else {
                            $feed_depot->addChild( $key, $value );
                        }
                    }
                }
                else {
    ...
    /**
     * @desc Calculate delivery/pickup time by adding
     * 2days with the given order deadline
     * @since 7.2.17
     * @param $order_deadline
     * @return string
     */
    private static function calculate_delivery_pickup_time( $order_deadline ) {
        return date('Y-m-d H:m', strtotime( $order_deadline . '+2 days' ) );
    }
