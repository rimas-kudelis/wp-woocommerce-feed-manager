...
namespace RexTheme\RexShoppingBestprice;

use SimpleXMLElement;
use RexTheme\RexShoppingBestprice\Item;
...
class Feed
{
    ...
    public function __construct($wrapper = false, $itemlName = 'item', $namespace = null, $version = '', $rss = 'rss', $stand_alone = false, $wrapperel = '', $namespace_prefix = '')
    {
        ...
        $this->feed = new SimpleXMLElement('<store></store>');
    }
    ...
    {   
        ...
        $this->feed->addChild('date',date('Y-m-d H:i'));
        ...
        foreach ($this->items as $item) {
            ...
            foreach ($item->nodes() as $key=>$itemNode) {
                if (is_array($itemNode)) {
                    ...
                    if($key == 'features'){
                        $feature = $feedItemNode->addChild('features');
                        $table = $feature->addChild('table');
                        $tr = $table->addChild('tr');
                        foreach ($itemNode as $node) {
                            $tr->addChild('td', $node->get('value'), $node->get('_namespace'));
                        }
                    }  
                    
                    if($key == 'imagesURL'){
                        $imagesURL = $feedItemNode->addChild('imagesURL');
                        $i = 1;
                        foreach ($itemNode as $node) {
                            $imagesURL->addChild('img'.$i, $node->get('value'), $node->get('_namespace'));
                            $i++;
                        }
                    }
                } else {
