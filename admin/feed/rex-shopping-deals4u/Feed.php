...
namespace RexTheme\RexShoppingDealsForU;

use SimpleXMLElement;
use RexTheme\RexShoppingDealsForU\Item;
            ...
            foreach ($item->nodes() as $itemNode) {
                    if (is_array($itemNode)) {
                        foreach ($itemNode as $node) {
                            ...
                            if(!(stristr($node->get('name'),'offer_additionalField_code_')) && !(stristr($node->get('name'),'offer_additionalField_value_'))  ){
                                $feedItemNode->addChild(str_replace(' ', '_', $node->get('name')), $node->get('value'), $node->get('_namespace'));
                            }
                        }
                    }else {
                        
                        if(!(stristr($itemNode->get('name'),'offer_additionalField_code_')) && !(stristr($itemNode->get('name'),'offer_additionalField_value_'))  ){
                            $itemNode->attachNodeTo($feedItemNode);
                        }
                    }
            }
            $feedItemNodeOffer = $feedItemNode->addChild('offer-additional-fields');
            foreach ($item->nodes() as $itemNode) {
                
                    if (!is_array($itemNode)) {
                        if(stristr($itemNode->get('name'),'offer_additionalField_code_')){
                            $this->param = $feedItemNodeOffer->addChild('offer-additional-field');
                            $this->param->addChild('code',$itemNode->get('value'));
                        }elseif(stristr($itemNode->get('name'),'offer_additionalField_value_')){
                            $this->param->addChild('value',$itemNode->get('value'));
                        }  
                    }
