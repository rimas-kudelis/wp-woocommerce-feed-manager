...
namespace RexTheme\ZapCoIl;

use SimpleXMLElement;
use RexTheme\ZapCoIl\Item;
...
class Feed
{
	 ...
	 * Feed constructor
	 */
	public function __construct($wrapper = false, $itemlName = 'item', $namespace = null, $version = '', $rss = 'rss', $stand_alone = false, $wrapperel = '', $namespace_prefix = '', $url = '', $date = '', $time = '' )
	{
		...
		$attributes .= $this->version && !empty($this->version) ? " version='$this->version'" : '';
		$attributes .= $this->url && !empty($this->url) ? " URL='$this->url'" : '';
		$attributes .= $this->date && !empty($this->date) ? " DATE='$this->date'" : '';
		$attributes .= $this->time && !empty($this->time) ? " TIME='$this->time'" : '';
		$stand_alone_text = $stand_alone ? 'standalone="yes"' : '';
		$this->feed = new SimpleXMLElement("<$rss $namespace $stand_alone_text $attributes ></$rss>");
	}
			...
			foreach ($item->nodes() as $itemNode) {
				if ( is_array( $itemNode ) ) {
					...
					if ( $itemNode->get( 'name' ) === 'id' ) {
						$feedItemNode->addAttribute( 'NUM', $itemNode->get( 'value' ));
					}
